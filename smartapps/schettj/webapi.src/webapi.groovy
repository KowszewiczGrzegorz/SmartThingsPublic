...
// on install
def installed() {
  log.debug "${title ?: location.name} WebAPI URL: ${generateURL("ui").join()}"
}

// on update
def updated() { 
  log.info "${title ?: location.name} WebAPI URL: ${generateURL("ui").join()}"
...
def findPresenceDiff(String id) {
    //log.debug "findPresenceDiff"        log.debug request
